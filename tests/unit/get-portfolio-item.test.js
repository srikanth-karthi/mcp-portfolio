...
describe('Get Portfolio Item Functionality', () => {
  ...
  describe('Error handling', () => {
    it('should throw McpError for non-existent ID', async () => {
      await expect(
        getPortfolioItem(portfolioData, { id: 999 })
    ...
    it('should throw error for negative ID', async () => {
      await expect(
        getPortfolioItem(portfolioData, { id: -1 })
      ).rejects.toThrow()
    })

    it('should throw error for ID 0', async () => {
      await expect(
        getPortfolioItem(portfolioData, { id: 0 })
      ).rejects.toThrow()
    })
  })
  ...
  describe('Edge cases', () => {
    it('should handle string ID that cannot be found', async () => {
      await expect(
        getPortfolioItem(portfolioData, { id: 'invalid' })
      ).rejects.toThrow()
    })

    it('should handle null ID', async () => {
      await expect(
        getPortfolioItem(portfolioData, { id: null })
      ).rejects.toThrow()
    })

    it('should handle undefined ID', async () => {
      await expect(
        getPortfolioItem(portfolioData, { id: undefined })
      ).rejects.toThrow()
    })
    ...
    it('should handle very large ID number', async () => {
      await expect(
        getPortfolioItem(portfolioData, { id: Number.MAX_SAFE_INTEGER })
      ).rejects.toThrow()
    })
  })
  ...
  describe('Different item types', () => {
    it('should retrieve Contact items correctly', async () => {
      const contactItem = mockPortfolioData.find(
        (item) => item.category === 'Contact'
      )
      if (contactItem) {
        const result = await getPortfolioItem(portfolioData, {
          id: contactItem.id
        })
        const parsedResult = JSON.parse(result.content[0].text)
    ...
    it('should retrieve Tech Stack items correctly', async () => {
      const techItem = mockPortfolioData.find(
    ...
    it('should retrieve Experience items correctly', async () => {
      const expItem = mockPortfolioData.find(
  ...
  describe('Empty data handling', () => {
    it('should throw error when portfolio is empty', async () => {
      ...
      await expect(
        getPortfolioItem(emptyPortfolioData, { id: 1 })
      ).rejects.toThrow(McpError)
    })
  })
})
